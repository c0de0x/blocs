...
CzBLOCSWallet::CzBLOCSWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBLOCSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBLOCSSeed_deprecated()) {
                    LogPrintf("%s: Updated zBLOCS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBLOCSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBLOCSCount(nCountLastUsed);
    else if (!walletdb.ReadZBLOCSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBLOCSWallet::Lock()
{
    seedMaster = 0;
}

void CzBLOCSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBLOCSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBLOCS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBLOCSWallet::LoadMintPoolFromDB()
{
...
void CzBLOCSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBLOCSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBLOCSWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBLOCSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBLOCS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBLOCSWallet::SeedToZBLOCS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBLOCSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBLOCSWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZBLOCSCount(nCountLastUsed);
...
void CzBLOCSWallet::GenerateDeterministicZBLOCS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBLOCSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBLOCSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBLOCSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
