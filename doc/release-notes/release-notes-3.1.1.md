...
### Fix spending for v1 zBTST created before block 1050020

The transition to v2 zBTST and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zBTST to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zBTST v1 will now be able to convert that into BTST and then zBTST v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zBTST stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by BITSTATS stakers.
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zBTST code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zBTST elements if needed. (presstab)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zBTST created before block 1050020. (presstab)
