...
On March 6th 2019, an attack was detected on the BITSTATS network zerocoin protocol, or zBTT. The vulnerability allows an attacker to fake serials accepted by the network and thus to spend zerocoins that have never been minted. As severe as it is, it does not harm users’ privacy and does not affect their holdings directly.

As a result of this, all zBTT functionality was disabled via one of our sporks shortly after verification of this exploit. A full report, detailing how this attack was performed, as well as investigation results and mitigation methods is available [On Medium](https://medium.com/@dev.bitstats/report-wrapped-serials-attack-5f4bf7b51701).

zBTT functions will be restored after v3.2.0 is pushed out and the majority of the network has upgraded.
...
This introduces the ability to do most of the heavy computation required for zBTT spends **before** actually initiating the spend. A new thread, `ThreadPrecomputeSpends`, is added which constantly runs in the background.

`ThreadPrecomputeSpends`' purpose is to monitor the wallet's zBTT mints and perform partial witness accumulations up to `nHeight - 20` blocks from the chain's tip (to ensure that it only ever computes data that is at least 2 accumulator checkpoints deep), retaining the results in memory.
...
Finally, the "security level" option for spending zBTT has been completely removed, and all zBTT spends now spend at what was formerly "security level" `100`. This change has been reflected in any RPC command that previously took a security level argument, as well as in the GUI's Privacy section for spending zBTT.
...
### GUI
 ...
 - #741 `ea2637838c` [GUI] Sort by 'data' in zBTT and coin control dialogs (random-zebra)
 - #733 `9a792d73e9` [GUI] Hide orphans (random-zebra)
 - #735 `44840c5069` [Qt] Stop using dummy strings in clientversion.cpp (Fuzzbawls)
 - #725 `793db15baf` [UI] Sort numbers correctly in zBTT and coin control dialogs (random-zebra)
 ...
 - #655 `de0c4e0888` [Qt] Fix BTT balances on overview page (Fuzzbawls)
...
### Wallet
 - #842 `c6c84fe85f` [Wallet] [zBTT] Precomputed Zerocoin Proofs (Fuzzbawls)
 ...
 - #759 `19fd0877cd` [Wallet] Avoid failed zBTT spend because of changed seed (warrows)
 - #755 `65be6b611b` [Wallet] Fix zBTT spend when too much mints are selected (warrows)
...
### Miscellaneous
 ...
 - #731 `f7f49cfa7c` [zBTT] Fix bignum overloads when using OpenSSL (Fuzzbawls)
